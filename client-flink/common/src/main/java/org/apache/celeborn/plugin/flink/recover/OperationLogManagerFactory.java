...
public class OperationLogManagerFactory {
  ...
  public static RecoverableStore createOperationLogManager(Configuration config) {
    if (Boolean.parseBoolean(config.getString(FLINK_JOB_RECOVERY_ENABLED, "false"))) {
      return new OperationLogManagerImpl();
    } else {
      return new DummyOperationLogManager();
    }
  }
}
